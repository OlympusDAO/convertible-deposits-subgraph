...
ponder.on("ConvertibleDepositAuctioneerLimitOrders:OrderChanged", async ({ event, context }) => {
  const chainId = Number(context.chain.id);
  const contractAddress = event.log.address as Address;
  const orderId = BigInt(event.args.orderId);
  ...
  // Record the OrderChanged event
  await context.db.insert(schema.limitOrderChanged).values({
    chainId,
    block: blockNumber,
    logIndex: event.log.logIndex,
    txHash: event.transaction.hash,
  ...
  // Update order: reset budget fields to new values
  await updateLimitOrder(context, chainId, contractAddress, orderId, {
    ...
    minFillSize,
    minFillSizeDecimal: toDecimal(minFillSize, assetDecimals),
    // Note: depositSpent/incentiveSpent are only on snapshots (cumulative), not on the order entity
  });

  // Get the latest previous snapshot to copy from
  // OrderChanged is cancel+create, so contract state resets, but we preserve cumulative totals
  const previousSnapshot = await getLatestLimitOrderSnapshot(
    context,
    chainId,
    contractAddress,
  ...
  // Create snapshot by copying previous (preserving cumulative spent amounts) and updating budgets/prices
  await createLimitOrderSnapshot(
    context,
    chainId,
    blockNumber,
    ...
    previousSnapshot,
    {
      depositBudgetDelta: depositBudget, // Update budget from event
      incentiveBudgetDelta: incentiveBudget, // Update budget from event
      maxPrice, // Update max price from event
      minFillSize, // Update min fill size from event
      // depositSpentDelta and incentiveSpentDelta default to 0, preserving previous cumulative values
    },
  );
});
